fn main() {
    ...
    */
    ...
    /*
        从Rust的角度讲，有三种形式用来了解字符串，分别是，字节，标量值（Unicode标量值），和字形簇（最接近人眼中字符的概念）
    */
    //字符串slice
    let hello = "Здравствуйте";
    let s = &hello[0..4];//s是一个&str字符串，它包含字符串的头四个字节。
    //但是如果字符串中有两个字节一个字符的情况但是你只索取一个字节的话，那么就会导致崩溃。
    /*遍历字符串的方法*/
    for i in hello.chars() { //chars()方法会返回每一个字符而不是一个字节一个字节的返回
        println!("{}", i);
    }
    for i in hello.bytes() { //bytes()方法返回每一个字节的值
        println!("{}", i);
    }
